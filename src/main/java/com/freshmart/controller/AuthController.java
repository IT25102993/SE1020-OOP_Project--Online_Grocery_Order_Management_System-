    ...
    public ResponseEntity<?> sendVerificationCode(@RequestBody Map<String, String> body) {
        try {
            ...
            if (email.toLowerCase().startsWith("admin:")) {
                isAdmin = true;
                actualEmail = email.substring(6).trim();
            }

            if (isAdmin) {
                if (adminFileService.existsByEmail(actualEmail)) {
                    return ResponseEntity.badRequest().body(Map.of("success", false, "message", "Admin email already registered"));
                }
            } else {
                if (customerFileService.existsByEmail(actualEmail)) {
    ...
    public ResponseEntity<?> registerCustomer(@RequestBody CustomerDto dto) {
        try {
            ...
            if (dto.getEmail().toLowerCase().startsWith("admin:")) {
                isAdmin = true;
                actualEmail = dto.getEmail().substring(6).trim();
            }
